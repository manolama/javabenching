...
public class Implementations {
  ...
  public static class StringTSByteId implements TSByteId {
    ...
    public void addMetric(final String m) {
      ...
      if (metrics.size() > 0) {
        Collections.sort(metrics, Bytes.MEMCMP);
      }
    }
    ...
    public void addAggTag(final String t) {
      ...
      if (aggregated_tags.size() > 0) {
        Collections.sort(aggregated_tags, Bytes.MEMCMP);
      }
    }
      ...
      try {
        ...
        if (tags != null) {
          for (final Entry<byte[], byte[]> pair : tags.entrySet()) {
            bais.write(pair.getKey());
            bais.write(pair.getValue());
          }
        }
        if (aggregated_tags != null) {
        ...
        bais.close();
        return (int) LongHashFunction.xx_r39().hashBytes(bais.toByteArray());
      } catch (IOException e) {
