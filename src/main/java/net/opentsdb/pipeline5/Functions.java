...
import net.opentsdb.pipeline5.TimeSortedDataStore;
import net.opentsdb.pipeline5.Implementations.*;
  ...
  public static class GroupBy implements TSProcessor, StreamListener, QResult, QExecutionPipeline {
    ...
    public void onNext(QResult next) {
      ...
      for (TS<?> ts : next.series()) {
        ...
        id.addMetric(new String(ts.id().metrics().get(0), Const.UTF8_CHARSET));
        id.addTag("host", new String(ts.id().tags().get("host".getBytes(Const.UTF8_CHARSET)), Const.UTF8_CHARSET));
        id.addAggTag("dc");
    ...
    public void onNext(QResult next) {
      ...
      for (TS<?> ts : next.series()) {
        ...
        for (Entry<byte[], byte[]> pair : ts.id().tags().entrySet()) {
          builder.addTag(new String(pair.getKey(), Const.UTF8_CHARSET), 
              new String(pair.getValue(), Const.UTF8_CHARSET));
        }
        for (byte[] tag : ts.id().aggregatedTags()) {
          builder.addAggTag(new String(tag, Const.UTF8_CHARSET));
        }
